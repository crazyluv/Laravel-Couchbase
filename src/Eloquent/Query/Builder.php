...
use Crazyluv\LaravelCouchbase\Eloquent\CouchbaseConnection;
use Crazyluv\LaravelCouchbase\Eloquent\Helper;
...
use Ytake\LaravelCouchbase\Query\Builder as CoucubaseBuilder;

class Builder extends CoucubaseBuilder
{
    /**
    ...
    public function insert(array $values)
    {
        ...
        if ($batch) {
            ...
            $result = $this->connection->getCouchbaseBucket()->upsert($this->key,
    ...
    public function pull($column, $value = null)
    {
        try {
            ...
            if ($e->getCode() === COUCHBASE_KEY_ENOENT) {
                trigger_error('Tying to pull a value from non existing document ' . json_encode($this->key) . '.',
                    E_USER_WARNING);
                return null;
            }
            throw $e;
        ...
        if (!isset($obj->value->{$column})) {
            trigger_error('Tying to pull a value from non existing column ' . json_encode($column) . ' in document ' . json_encode($this->key) . '.',
                E_USER_WARNING);
            return null;
        }
