...
use Ytake\LaravelCouchbase\Query\Grammar as YGrammar;
...
class Grammar extends BaseGrammar
{
    ...
    /**
     * @param \Crazyluv\LaravelCouchbase\Eloquent\Query\Builder $query
     * @return string
     */
     ...
     * @throws Exception
     */
    public function compileUpsert(Builder $query, array $values): string
    {

        // keyspace-ref:
        $table = $this->wrapTable($query->from);
        // use keys/index clause:
        $useClause = $this->wrapKey($query->key);
        // returning-clause
        $returning = $this->compileReturning($query);
        ...
        $parameters = [];
        foreach ($values as $key => $value) {
            $parameters[] = '(' . $this->parameterize($value) . ')';
        }

        $parameters = (!$useClause) ? implode(', ', $parameters) : "({$useClause}, \$parameters)";
        $keyValue = (!$useClause) ? null : '(KEY, VALUE)';
    ...
    {
        if ($query->key !== null && !empty($query->indexes)) {
            throw new Exception('Only one of useKeys or useIndex can be used, not both.');
        }
        if ($query->key !== null) {
            return 'USE KEYS ' . $this->wrapData($query->key);
        }
